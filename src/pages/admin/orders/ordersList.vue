<template>
  <v-card flat>
    ...
    <VCol cols="12" sm="4" md="4" class="d-flex justify-end">
              <v-btn color="primary" :to="{ name: 'order add' }" class="justify-end">
                <VIcon />Agregar pedido
              </v-btn>
            </VCol>
...
const loadOrder = async() =>{
  try {
    await orderStore.getCategory();
    items.value = orderStore.categories;
  } catch (error) {
...
const confirmDelete = async () => {
  if (selectedItem.value) {
    try {
      await orderStore.deleteCategory(selectedItem.value.id);
